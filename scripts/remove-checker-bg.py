...
def remove_checker_bg(path: Path) -> tuple[int, int]:
    ...
    # 既に透明なピクセル / 任意の濃さのグレー全てを bg_candidate に含めて連結性を確保。
    # キャラ内の純黒アウトライン (r<20 で is_gray=True だが) も含まれるが、
    # 4 隅起点の連結成分判定でキャラ周辺の透明帯と分離されるので保護される。
    is_gray = (np.abs(r - g) < 30) & (np.abs(g - b) < 30) & (np.abs(r - b) < 30)
    is_already_transparent = a < 128
    # 暗いチェッカー柄 (RGB~0-15 純黒系) と明るいチェッカー柄 (RGB>=80 灰色系) のペアを取る。
    # 中間色 (16-79) は除外: キャラ内シャドウ/瞳の濃い色を背景候補にしない。
    # キャラ周辺の肌色 (R>>G>B でグレー判定 False) がバリアになりキャラ内黒線は保護される。
